...
class Network(val inputs: List[CodingSeq], userid: Int, val networkname: String, stepSize: Double = 1) {
  ...
  def simulate(finish: Double): List[List[(String, Double, Double)]] = {
    ...
    def getConcs(l: List[CodingSeq] = inputs): Set[(String, Double, Double)] = l.flatMap(seq => seq match {
      case CodingSeq(name, _, _, _) if (!seq.ready) => {
        seq.ready = true;
  ...
  def simJson(finish: Double) = {
    ...
    val json = Json.toJson(flipped.values.flatMap(dataset => {
      ...
      List(Json.obj("name" -> Json.toJson("mRNA_" + dataset(0)._1), "data" ->
        dataset.map(_._2).map(conc => {
          x += stepSize; Json.obj("x" -> x, "y" -> conc)
        })),
        Json.obj("name" -> Json.toJson("protein_" + dataset(0)._1), "data" ->
          dataset.map(_._3).map(conc => {
            y += stepSize; Json.obj("x" -> y, "y" -> conc)
          }))
      )
    }))
    (json, file)
  ...
  def step() {
    ...
    inputs.foreach(x => {
      if (x.linkedBy.isEmpty) {
        x.ready = true; x.concentration = x.concentration.tail
      }
    })
    ...
    def solve(odePairs: List[ODEPair]): List[VectorD] = {
      odePairs.map({
        case (ode, concs) if concs.length == 3 => RungeKutta.integrateVV(Array((d, v) => 0.0, (d, v) => ode(d, v)(0), (d, v) => ode(d, v)(1)), concs, stepSize, 0.0, stepSize)
  ...
  def getNetworks(userId: Int) = {
    DB.withConnection {
      implicit connection =>
        ...
        val resMap = Map(networks map {
          s => (s, Network.load(userId, s))
        }: _*)
