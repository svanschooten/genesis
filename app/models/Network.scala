    ...
    def step() {
        ...
        def reachable(level: List[CodingSeq], seq: CodingSeq): Boolean = level.foldLeft(false)((soFar: Boolean, cs: CodingSeq) => soFar || (seq == cs || (cs match {
            case CodingSeq(_,_,_,link) => link match {
                case None => false
                case Some(NotGate(_,nextSeq,_,_,_)) => reachable(List(nextSeq),seq)
                case Some(AndGate((seq,seq1),_,_,_,_)) if seq != seq1 => false
                case Some(AndGate((seq1,seq),_,_,_,_)) if seq != seq1 => false
                case Some(AndGate((_,_),nextSeq,_,_,_)) => reachable(List(nextSeq),seq)
                case _ => false
            }
            case _ => false
        })))
        ...
        def partition(css: List[CodingSeq]): (List[CodingSeq],List[CodingSeq]) = css.partition( _ match {
            case CodingSeq(_,_,_,link) => link match {
                ...
                case Some(AndGate((seq1,seq2),_,_,_,_)) if reachable(inputs,seq1) && reachable(inputs,seq2) => true
                case _ => false
            }
            case _ => false
        })
