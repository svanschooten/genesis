 ...
 *------------------------------------------------------------------------------
 *  The ODE is of the form y(t)' =  y'(t) = f(t, y)
 *  with initial condition y0 = y(t0).
 *------------------------------------------------------------------------------
 *  If f is a linear function of the form a(t) * y(t) + b(t), then the ODE is
 *  linear, if a(t) = c (i.e., a constant) the ODE has constant coefficients and
...
trait Integrator {
     ...
     *  Ordinary Differential Equations (ODEs) where [f_j(t, y_j)] is an array of
