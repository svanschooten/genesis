...
class VectorD (val dim: Int, protected var v: Array[Double] = null)
    ...
    if(v == null)
        ...
        println("constructor", "dimension is wrong")
    ...
    def expand(more: Int = dim): VectorD = {
        if(more < 1)
            ...
            new VectorD(dim + more, Array.concat(v, new Array[Double](more)))
    }
    ...
    def ++ (b: VectorD): VectorD = {
        ...
        for(i <- c.range)
            c.v(i) = if (i < dim) v(i) else b.v(i - dim)
        c
    }
    ...
    def + (s: (Double, Int)): VectorD = {
        val c = new VectorD(dim)
        for(i <- range)
            c.v(i) = if (i == s._2) v(i) + s._1 else v(i)
        c
    }
    ...
    def max(b: VectorD): VectorD = {
        val c = new VectorD(dim)
        for(i <- range)
            c.v(i) = if(b.v(i) > v(i)) b.v(i) else v(i)
        c
    }
