...
import scalation.stat.Regression
 ...
 */
class QuadraticFit (f: FunctionV2S [Double], n: Int = 2)
{
     ...
     *  the surface, i.e., determine the cofficients of the regression equation.
     *  @param result  the design matrix and response vector grouped in a tuple
     */
    def fit (result: Tuple2 [MatrixD, VectorD])
    {
        val reg = Regression.fit (result._1, result._2)
        b = reg._1                               // cofficients in regression equation
        println ("fit: R-squared = " + reg._2)   // cofficient of determination
    } // fit
